...
function getInfoCountry($idpays)
{
        global $pdo;
    $requete = "SELECT * FROM Country where id = :id";

    $prep = $pdo->prepare($requete);
    $prep->bindValue(':id', $idpays);
    $prep->execute();
...
function getCountriesByContinent($continent)
{
    ...
    global $pdo;
    $query = 'SELECT * FROM Country WHERE Continent = :cont ORDER BY id';
...
function getcapitale($numCapitale)
{
    // pour utiliser la variable globale dans la fonction
    global $pdo;
    $query = 'SELECT * FROM city WHERE id = :num ';
    $prep = $pdo->prepare($query);
    // on associe ici (bind) le paramètre (:cont) de la req SQL,
    // avec la valeur reçue en paramètre de la fonction ($continent)
    // on prend soin de spécifier le type de la valeur (String) afin
    // de se prémunir d'injections SQL (des filtres seront appliqués)
    $prep->bindValue(':num', $numCapitale, PDO::PARAM_STR);
    $prep->execute();
    // var_dump($prep);  pour du debug
    // var_dump($continent);
